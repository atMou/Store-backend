...
public class CouponsController(ISender sender) : ControllerBase
{
    ...
    [HttpGet]
    [Route("user/{id}")]
    public async Task<ActionResult<PaginatedResult<CouponResult>>> GetByUserId(
        [FromRoute] Guid id,
        [FromQuery] GetCouponsByUserIdRequest request)
    {
        var result = await sender.Send(new GetCouponsByUserIdQuery(
            UserId.From(id),
            request.PageNumber,
            request.PageSize,
            request.Status
    ...
    public async Task<ActionResult<Unit>> Create([FromBody] CreateCouponCommand request)
    {
