...
) : IConsumer<CartItemsPriceChangedIntegrationEvent>
{
    public async Task Consume(ConsumeContext<CartItemsPriceChangedIntegrationEvent> context)
    {
        logger.LogInformation(
            "Processing CartItemsPriceChangedIntegrationEvent for Id: {Id}, NewPrice: {NewPrice}",
        ...
        result.Match(
            Succ: _ => logger.LogInformation(
                "Successfully updated cart items price for Id: {Id}",
                context.Message.ProductId),
            Fail: async err =>
            {
                logger.LogError(
                    "Failed to update cart items price for Id: {Id}. Error: {@Error}",
                    context.Message.ProductId,
                    err);
