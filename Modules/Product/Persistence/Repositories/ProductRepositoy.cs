...
public class ProductRepository : IProductRepository
{
        ...
        GetProductsQuery query)
    {
        ...
        return IO.liftAsync(async e =>
                     await ctx.Products.WithQueryOptions(options => { ProductOptionsEvaluator(options, query); })
                         .GroupBy(_ => 1)
                         .Select(g => new { TotalCount = g.Count(), Items = g.ToList() })
                         .FirstOrDefaultAsync(e.Token)).Map(res => (res!.Items, res.TotalCount));
    }
    ...
    public IO<Domain.Models.Product> UpdateProduct(UpdateProductDto productDto,
        ProductDBContext ctx, Action<QueryOptions<Domain.Models.Product>>? options = null)
    {
        return from product in GetProductById(productDto.ProductId, ctx, options)
               from variants in GetProductsByIds(productDto.VariantsIds, ctx)
               from updatedProduct in product.Update(productDto, variants).Match(
                   prod => IO.lift(() => prod),
                   IO.fail<Domain.Models.Product>)
               select updatedProduct;
    }
    ...
    private QueryOptions<Domain.Models.Product> ProductOptionsEvaluator(QueryOptions<Domain.Models.Product> options, GetProductsQuery query)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.Brand))
            options = options.AddFilter(p => p.Brand.Name.Contains(query.Brand));

        if (!string.IsNullOrWhiteSpace(query.Category))
            options = options.AddFilter(p => p.Category.Name.Contains(query.Category));

        if (!string.IsNullOrWhiteSpace(query.Color))
            options = options.AddFilter(p => p.Color.Name.Contains(query.Color));

        if (!string.IsNullOrWhiteSpace(query.Size))
            options = options.AddFilter(p => p.Size.Name.Contains(query.Size));

        if (!string.IsNullOrWhiteSpace(query.Search))
            options = options.AddFilter(p => p.Slug.Value.Contains(query.Search));

        if (query.MinPrice.HasValue)
            options = options.AddFilter(p => p.Price.Value >= query.MinPrice.Value);

        if (query.MaxPrice.HasValue)
            options = options.AddFilter(p => p.Price.Value <= query.MaxPrice.Value);
