	...
	ILogger<UserCreatedIntegrationEventHandler> logger) : IConsumer<UserCreatedIntegrationEvent>
{
				 ...
				 let emailBody = template
					 .Replace("{{VerificationLink}}",
						 BuildVerificationLink(
							 ...
							 context.Message.VerificationToken!,
							 "/"
						 ))
					 .Replace("{{UserEmail}}", context.Message.Email)
					 .Replace("{{code}}", context.Message.VerificationCode)
					 .Replace("{{name}}", context.Message.Name)
	...
	{
		return
			$"http://localhost:3000/users/verify?email={email}&token={token}&returnUrl={Uri.EscapeDataString(returnUrl)}";
	}
}