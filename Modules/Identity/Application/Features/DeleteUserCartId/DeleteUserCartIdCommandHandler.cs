...
{
    public async Task<Fin<Unit>> Handle(DeleteUserCartIdCommand command, CancellationToken cancellationToken)
    {
        var db =
    ...
    private IO<Unit> EnsureIsSameUserOrAdmin(UserId userId) =>
        ...
        | userContext.HasRole<IO>(Role.Admin, UnAuthorizedError.New($"You are not Authorized to Delete User Cart Id"))
            .As();
}
